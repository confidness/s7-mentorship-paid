...
import { ArrowLeft, CheckCircle2, Circle, Code2, FileText, ListChecks, MessageSquareText, Plus, Save, Trash2, Upload, Wallet, X } from 'lucide-react'
...
export default function LessonBuilder() {
  ...
  const [paid, setPaid] = useState((existing?.priceCents ?? 0) > 0)
  const [priceText, setPriceText] = useState(existing?.priceCents ? String(existing.priceCents / 100) : '')
  ...
  function validate() {
    ...
    if (paid) {
      const cents = parsePrice(priceText, currency)
      if (cents === null || cents <= 0) next.price = t('enter_a_price_above_zero')
    }
  ...
  function persist(publish: boolean) {
    ...
    const priceCents = paid ? (parsePrice(priceText, currency) ?? 0) : 0
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="space-y-5 p-6">
        <SectionHeading title={t('price')} subtitle={t('free_or_paid_you_keep_most_of_each_sale')} icon={Wallet} />

        <fieldset>
          <legend className="sr-only">{t('price')}</legend>
          <div className="grid grid-cols-2 gap-2.5">
            {([false, true] as const).map((isPaid) => (
              ...
              >
                {isPaid ? t('paid_lesson') : t('free_lesson')}
              </button>
            ))}
          </div>
        </fieldset>
            ...
            <p id="price-split" className="fill-soft px-3.5 py-3 text-sm text-ink-600">
              {priceCents > 0
                ? t('you_receive_x_of_y_after_the_platform_fee', {
                    ...
                    total: formatMoney(priceCents, currency, locale),
                  })
                : t('enter_a_price_above_zero')}
            </p>
